  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['facebook:svip:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['facebook:svip:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="达标金额" prop="standard">
          <el-input v-model="form.standard" placeholder="请输入达标金额" />
        </el-form-item>
        <el-form-item label="增幅比例" prop="rate">
...
<script>
import { listVip, getVip, delVip, addVip, updateVip } from "@/api/facebook/svip";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        standard: [
          { required: true, message: "达标金额不能为空", trigger: "blur" }
        ],
        rate: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除svip编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delVip(ids);
      }).then(() => {
        this.loading = false;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('facebook/svip/export', {
        ...this.queryParams
